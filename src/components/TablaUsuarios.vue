          ...
          <tr v-for="usuario in usuariosPorPagina" :key="usuario.id">
            ...
            <td class="align-middle text-center">
              <span v-if="tiposUsuario.length > 0">
                {{ getTipoUsuario(usuario.tipoUsuario) }}
              </span>
            </td>
            <td v-if="verHistorico" class="align-middle text-center">
              {{ usuario.baja }}
            </td>
...
export default {
  ...
  computed: {
    usuariosFiltrados() {
      ...
      return this.verHistorico
        ? this.usuarios
        : this.usuarios.filter((usuario) => !usuario.baja); //lo hace todo
    },
    usuariosPorPagina() {
  ...
  methods: {
        ...
        try {
          ...
          if (usuarioExistente && usuarioExistente.baja !== "") {
            ...
            const crearResponse = await fetch(
              `http://localhost:3000/usuarios/${usuarioExistente.id}`,
              {
                method: "PUT",
                headers: {
            ...
            if (!crearResponse.ok) {
              throw new Error(
                `Error al dar de alta el usuario: ${crearResponse.statusText}`
              );
            }
        ...
        if (this.usuario.dni) {
          try {
            ...
            const response = await fetch(
              `http://localhost:3000/usuarios/${this.usuario.id}`,
              {
                method: "PUT",
                headers: {
                  "Content-Type": "application/json",
            ...
            if (!response.ok) {
              throw new Error(
                `Error al modificar el usuario: ${response.statusText}`
              );
            }
    ...
    async seleccionarUsuario(usuario) {
      try {
        ...
        if (usuarioEncontrado) {
          ...
          if (this.usuario.provincia) {
            this.usuario.provincia = this.provincias.find(
              (p) => p.nm === this.usuario.provincia
            ).nm;
            if (this.usuario.provincia) {
          ...
          if (this.usuario.fechaAlta) {
            this.usuario.fechaAlta = this.usuario.fechaAlta.toLocaleString(
              "es-ES",
              {
                day: "2-digit",
                month: "2-digit",
    ...
    getTipoUsuario(tipoUsuarioID) {
      return this.tiposUsuario.find((tipoUser) => tipoUser.id === tipoUsuarioID)
        .tipo;
    },
